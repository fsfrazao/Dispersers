...
class Dispersers_World(World):
    ...
    def setup_database(self, filename, sim_number):
        ...
        try:
            self.db.get_node(
                "/sim_{0}/dispersers/sys_lvl/Seeds".format(sim_number))
        except NoSuchNodeError:
            database_utils.create_tables(self.db, sim_number)

        self.seed_table = self.db.get_node(
            "/sim_{0}/dispersers/sys_lvl/Seeds".format(sim_number))
        self.ind_table

    def add_seed_entry(self, tree_id, initial_x,
                       initial_y, final_x, final_y, dispersal_type):
        seed_table = self.db.get_node(
            "/sim_{0}/dispersers/sys_lvl/Seeds".format(self.sim_number))
    ...
    def add_pop_entry(self):
        pop_table = self.db.get_node(
            "/sim_{0}/dispersers/sys_lvl/Populations".format(self.sim_number))
        pop_r = pop_table.row
    ...
    def add_ind_entry(self, ind_id, initial_x,
        ...
        ind_table = self.db.get_node(
            "/sim_{0}/dispersers/ind_lvl/Ind".format(self.sim_number))
        ind_r = ind_table.row
    ...
    def create_dispersers(self, n, disperser_class=PrimaryDisperser):
        trees_for_dispersers = np.random.choice(
            list(self.parent_tree_class.Instances.keys()), size=n, replace=True)

        for i in range(n):
            tree_id = trees_for_dispersers[i]
            pos = self.parent_tree_class.Instances.get(tree_id).position
            disperser_class(position=pos, world=self, gut_passage_time=2,
                            max_fruits=5, current_tree_id=tree_id)
    ...
    def model_status_from_file(self, input_file):
        ...
        for ft in self.parent_tree_class.DERIVED_TREES.keys():
            ...
            self.create_trees(self.parent_tree_class.DERIVED_TREES[ft], len(
                trees[ft]), pos=positions, ids=ids, world=self)

        self.topology.update()
        # for t in Tree.Instances.values():
        #     t.update()
        #     t.AGB=t.AGB_from_DBH(t.DBH/100)
        self.parent_tree_class.ID = max(
            self.parent_tree_class.Instances.keys()) + 1

    def model_status_to_file(self, output_file):
        trees_per_type = {k: []
                          for k in self.parent_tree_class.DERIVED_TREES.keys()}
