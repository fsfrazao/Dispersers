...
class PrimaryDisperser(Agent):
    ...
    def defecate(self, seed):
        ...
        x1, y1 = self.position
        seed_pos = self.world.topology.rand_point_along_line(
            x0=round(x0, 2), y0=round(y0, 2), x1=round(x1, 2), y1=round(y1, 2), offset=2)
    ...
    def add_seed(self, tree_pos, tree_type, tree_id):
        ...
        self.seeds.append({"time": self.world.step, "tree_type": tree_type,
                           "tree_id": tree_id, "tree_pos": tree_pos})

    def trees_within_radius(self, radius):
        cell_length = self.world.topology.patch_area**0.5

        neighborhood = self.world.topology.hood(
            cell=self.patch, remove_center=False, radius=round(radius / cell_length))
        trees_in_neighborhood = self.world.topology.trees_in_patches(
            neighborhood)
        trees_in_circle = [id for id, p in trees_in_neighborhood if self.world.topology.point_within_circle(
            self.position[0], self.position[1], radius, p[0], p[1], self.world.topology.scaled_distance, scale=cell_length)]
    ...
    def feeding_tree_value(self, tree):
        ...
        distance = self.world.topology.euclidean_distance(
            self.position[0], self.position[1], tree.position[0], tree.position[1])

        gap = self.world.topology.line_between_points(
            self.patch[0], self.patch[1], tree.patch[0], tree.patch[1])
        x = [g[0] for g in gap]
        y = [g[1] for g in gap]
    ...
    def choose_feeding_tree(self, trees):
        if len(trees) > 0:
            tree_values = []
            for tree in trees:
                tree_value = self.feeding_tree_value(
                    self.world.parent_tree_class.Instances.get(tree))
                tree_values.append({'tree_id': tree, 'tree_value': tree_value})

            sorted_by_value = sorted(
                tree_values, key=lambda k: k['tree_value'], reverse=True)
    ...
    def choose_non_feeding_tree(self, trees):
        if len(trees) > 0:
            tree_values = []
            for tree in trees:
                tree_value = self.non_feeding_tree_value(
                    self.world.parent_tree_class.Instances.get(tree))
                tree_values.append({'tree_id': tree, 'tree_value': tree_value})
    ...
    def activity_go_to_feeding_tree(self):
        ...
        try:
            chosen_tree = self.world.parent_tree_class.Instances.get(
                self.choose_feeding_tree(trees))
    ...
    def activity_roam(self):
        ...
        if len(trees) >= 1:
            chosen_tree = self.world.parent_tree_class.Instances.get(self.choose_non_feeding_tree(
                trees))  # self.world.parent_tree_class.Instances.get(np.random.choice(trees))
            self.move(pos=chosen_tree.position)
            self.current_tree_id = chosen_tree.id
